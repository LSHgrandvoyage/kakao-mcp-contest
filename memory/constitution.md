...
### 3-1. 사전적재 원칙 (Precompute-first — 100ms 준수를 위한 설계 규칙)

> 근거: 외부 공공 API(국토부·건축물대장·HUG 등)는 응답이 수백ms~초 단위라
> 요청 시점에 직접 호출하면 평균 100ms를 사실상 위반한다.

- [ ] **요청 핫패스에서 외부 API 직접 호출 금지.** tool 호출 경로는 **로컬 데이터스토어 조회만** 한다.
- [ ] 외부 데이터는 **스케줄러/배치로 사전 적재(precompute)** 하여 로컬에 materialize 한다.
      (요청이 오면 그때 가져오는 지연 캐싱(cache-aside) **불가** — 롱테일 신규 키가 MISS로 평균을 깨뜨림)
- [ ] 로컬 조회는 **인덱스 필수** (주소/좌표/식별자 등 조회 키).
- [ ] 불가피하게 실시간 외부 호출이 필요한 경로가 생기면, 그 tool은 **별도로 격리**하고
      p99 3s 이내를 보장하는 **타임아웃·폴백**을 명시한다. (평균 100ms 대상에서 제외 설계)
- [ ] 정적 콘텐츠 기반 tool(체크리스트·특약 문구 등)은 외부 의존 없이 **로컬 상수/문서**로 응답한다.
